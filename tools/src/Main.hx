...
class Main {
	static function main() {
		...
		var proc:Process = new Process("haxelib", ["path", "hl_glad"]);
		var code = proc.exitCode();
		if (code != 0) {
			Sys.println("Error getting the hl_glad path: " + proc.stderr.readAll().toString());
			Sys.exit(code);
		}
		...
		var bytes:Bytes = File.getBytes("./glad.hdll");
		File.saveBytes(Path.join([original, "glad.hdll"]), bytes);
	}
